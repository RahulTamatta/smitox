...
import 'package:eshop_multivendor/Model/Tax.dart';
...
class CartProvider extends ChangeNotifier {
  ...
  String? instamojoPaymentMode,
      instamojoClientId,
      instamojoClientSecretKey,
      instamojoOrderCreationUrl;
  ...
  // Getter methods for status and percentage
  int _currentQuantity = 0;

  // Getter method for currentQuantity
  int get currentQuantity => _currentQuantity;

  // Setter method for currentQuantity
  ...
  String? get selectedPe => _selectedMe;

  setSelectedMethod(String method) {
    _selectedMe = method;
    notifyListeners();
  }

  double? sellingPrice; // Add this line to declare sellingPrice

  // ... existing methods ...

  // Add getter and setter methods for sellingPrice if needed
  ...
  double? get sellingPrices => sellingPrice;

  String? get getStatus => status;
  String? get getPercentage => percentage;
  Future<void> getTax(BuildContext context) async {
    try {
      var id = context.read<UserProvider>().userId!;

      // Make API call
      var result = await ApiBaseHelper().getTaxData(id);

      // Directly use the result as a Tax object
      Tax tax = result as Tax;

      // Access status and percentage
      status = tax.taxDetail?.first.status;
      percentage = tax.taxDetail?.first.percentage;
      print('Status and percentage:$status and $percentage');
  ...
  // Getter method for productAmount
  double? get productAmount => _productAmount;

  // Setter method for productAmount
  set productAmount(double? value) {
    _productAmount = value;
    notifyListeners();
  }

  // Method to calculate product amount
  void calculateProductAmount() {
    if (sellingPrice != null) {
      productAmount = currentQuantity * sellingPrice!;
    } else {
      productAmount = null; // Set to null if sellingPrice is not available
    }
  }
      ...
      try {
        ...
        if (!error) {
          ...
          if (IS_SHIPROCKET_ON == '0') {
            ...
            if (_selectedMe == 'advance') {
              print('This is cartprovider $_selectedMe');
              double percentage = double.parse(getPercentage ?? '0');
              totalPrice = (totalPrice * percentage) /
                  100; // Apply the percentage calculation
            } else {
              totalPrice;
            }
          } else {
      ...
      try {
        ...
        if (!error) {
          ...
          if (move == false) {
            ...
            double deliveryCharge = 0.0; // Initialize to a default value
            if (data.containsKey('delivery_charge')) {
              var deliveryChargeString = data['delivery_charge'].toString();
              try {
                deliveryCharge = double.parse(deliveryChargeString);
              } catch (e) {
                print('Error parsing delivery charge: $e');
                // Handle the error appropriately, e.g., set deliveryCharge to a default value
                // deliveryCharge = 0.0;
              }
            }

// Now you can use deliveryCharge in your calculations
            totalPrice = deliveryCharge + oriPrice;
