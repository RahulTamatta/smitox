...
class AddressProvider extends ChangeNotifier {
  ...
  Future<void> addNewAddress(BuildContext context, Function updateNow,
    ...
    try {
      ...
      print('parameter update $parameterAdd and$update');
      if (update == true) {
        User item = context.read<CartProvider>().addressList[index];
        String? mobileC = item.mobile!;
        String? nameC = item.name!;
        String? altMobC = item.altMob!;
        // String? landmarkC = item.landmark!;

        String? addressC = item.address!;
        String? stateC = item.state!;
        String? countryC = item.country!;

        String? cityC = item.landmark!;
        print('City $cityC');
        String? areaC = item.pincode!;
        ...
        var parameter = {
          ...
          NAME: name ?? nameC,
          MOBILE: mobile ?? mobileC,
          PINCODE: areaName ?? areaC,
          // LANDMARK: city ?? cityC,
          // CITY: city,
          //CITY_ID: city,
          //AREA_ID: area,
          ADDRESS: address ?? addressC,
          STATE: state ?? stateC, COUNTRY: country ?? countryC,
          TYPE: type,
          ISDEFAULT: checkedDefault.toString() == 'true' ? '1' : '0',
        };

        dynamic result = await AddressRepository.addAndUpdateAddress(
          parameter: update ? parameter : parameterAdd,
          update: update,
        );
        print('parameter update  and $update');
        // Print the response body and other relevant information
        print('Response from addAndUpdateAddress: $result');

        bool error = result['error'];
        String? msg = result['message'];

        // if (update) {
        //   parameterAdd[ID] = context.read<CartProvider>().addressList[index].id;
        // }
        // if (cityName != null) {
        //   parameterAdd['city_name'] = cityName;
        //   parameterAdd[CITY_ID] = '0';
        // } else {
        //   // Handle the case when cityName is null
        //   print('City name is null');
        // }

        // if (zipcodeName != null) {
        //   parameterAdd['pincode_name'] = zipcodeC!.text;
        //   // parameter[SYSTEM_PINCODE] = '0';
        // } else {
        //   // Handle the case when zipcodeName is null
        //   print('Zipcode name is null');
        // }
