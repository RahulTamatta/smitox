...
import 'package:eshop_multivendor/Screen/Product%20Detail/productss.dart';
// import 'package:eshop_multivendor/Screen/Product%20Detail/quantitySelector.dart';
...
class StateItem extends State<ProductDetails> with TickerProviderStateMixin {
  int globalIndex = 0; // Define a global index variable
  ...
  void initState() {
    ...
    print('fetched  minimum*****${widget.model!.productSets}');
    if (widget.model!.productSets != null &&
        widget.model!.productSets!.isNotEmpty) {
      // Product sets are fetched
      for (ProductSet productSet in widget.model!.productSets!) {
        print('Product Set ID: ${productSet.id}');
        print('Minimum Quantity: ${productSet.minimumQuantity}');
        print('Maximum Quantity: ${productSet.maximumQuantity}');
        print('Discount MRP: ${productSet.discountMrp}');
        print('Discount Price Set: ${productSet.discountPriceSet}');
        print('Selling Price Set: ${productSet.sellingPriceSet}');
        print('Created At: ${productSet.createdAt}');
        print('Status: ${productSet.status}');
      }
    } else {
      // Product sets are not fetched
      print('Product sets not fetched for this product.');
    }
  ...
  // void fetchMinimumQuantity() {
  //   // Simulated API call or any asynchronous operation to fetch data
  //   // Replace this with your actual API call
  //   // For demonstration purposes, using a delayed future
  //   Future.delayed(const Duration(seconds: 1), () {
  //     // Update the state with the fetched minimum quantity
  //     setState(() {
  //       minimumQuantity = widget.model!.productSets as String?;
  //     });
  //   });
  // }
  ...
  // void updateNewQuantity(int quantity) {
  //   setState(() {
  //     newQty = quantity;
  //   });
  // }

  removeFromCartDetail(int index) async {
    isNetworkAvail = await isNetworkAvailable();
    if (isNetworkAvail) {
      if (context.read<UserProvider>().userId != '') {
        ...
        qty = (int.parse(controllerText[index].text) +
            int.parse(widget.model!.prVarientList![0].unit_set
                .toString()
                .toString()));
        ...
        var parameter = {
          PRODUCT_VARIENT_ID: widget.model!.prVarientList![0].id,
        ...
        if (qty < widget.model!.minOrderQuntity!) {
          qty = 0;
          db.removeCart(
              widget.model!.prVarientList![0].id!, widget.model!.id!, context);
          context
              .read<CartProvider>()
              .removeCartItem(widget.model!.prVarientList![0].id!);
        } else {
          context.read<CartProvider>().updateCartItem(widget.model!.id!,
              qty.toString(), 0, widget.model!.prVarientList![0].id!);
          db.updateCart(
            widget.model!.id!,
  ...
  Future<void> addToCartDeatail(int index, String qty, int from) async {
    isNetworkAvail = await isNetworkAvailable();
    if (isNetworkAvail) {
      if (context.read<UserProvider>().userId != '') {
        if (mounted) {
        ...
        if (qty.isNotEmpty && int.tryParse(qty) != null) {
          int qtyInt = int.parse(qty);
          int minOrderQty = int.parse(widget.model!.minOrderQuntity.toString());

          if (qtyInt < minOrderQty) {
            qty = minOrderQty.toString();
            setSnackbar("${getTranslated(context, 'MIN_MSG')}$qty", context);
          }
        } else {
          // Handle the case where qty is not a valid integer
          print('Exception caught');
          // You can log an error, show an alert, or take appropriate action here.
        }
        ...
        if (singleSellerOrderSystem) {
            ...
            if (from == 1) {
              ...
              context.read<CartProvider>().addCartItem(
                    SectionModel(
                      qty: qty,
                      productList: prList,
                      varientId: widget.model!.prVarientList![0].id!,
                      id: widget.model!.id,
              ...
              print(
                  'Hehehe ${widget.model!.prVarientList![0].id!}and${widget.model!.id} ');
              db.insertCart(
                widget.model!.id!,
                widget.model!.prVarientList![0].id!,
                qty,
              ...
              setSnackbar(
                  "${getTranslated(context, 'MAXQTY')!} ${widget.model!.itemsCounter!.last}",
                  context);
            } else {
              if (int.parse(qty) >
                  int.parse(widget.model!.itemsCounter!.last)) {
                setSnackbar(
                    "${getTranslated(context, 'MAXQTY')!} ${widget.model!.itemsCounter!.last}",
                    context);
              } else {
                context.read<CartProvider>().updateCartItem(widget.model!.id!,
                    qty, 0, widget.model!.prVarientList![0].id!);
                db.updateCart(
                  widget.model!.id!,
                  widget.model!.prVarientList![0].id!,
                  qty,
          ...
          if (from == 1) {
            ...
            if (int.parse(qty) > int.parse(widget.model!.itemsCounter!.last)) {
              setSnackbar(
                  "${getTranslated(context, 'MAXQTY')!} ${widget.model!.itemsCounter!.last}",
                  context);
            } else {
  ...
  Widget build(BuildContext context) {
    if (widget.index < productList.length) {
      ...
      Future<void> addToCart(int index, String qty, int from) async {
        ...
        if (isNetworkAvail) {
          if (context.read<UserProvider>().userId != '') {
            ...
            if (int.parse(qty) < widget.model!.minOrderQuntity!) {
              qty = widget.model!.minOrderQuntity.toString();
              ...
              if (CurrentSellerID == '' ||
                ...
                if (from == 1) {
                  ...
                  if (int.parse(qty) >
                    ...
                    setSnackbar(
                        getTranslated(context, 'Cart Update Successfully')!,
                        context);
                  }
                }
              } else {
      ...
      //       if (qty < widget.model!.minOrderQuntity!) {
      //         qty = 0;
      //         db.removeCart(widget.model!.prVarientList![0].id!,
            ...
            ? _isLoading
                ...
                : Consumer<CartProvider>(builder: (context, data, _) {
                    final tempId = data.cartList.firstWhereOrNull((cp) =>
                        cp.id == widget.model?.id &&
                        cp.varientId == widget.model?.prVarientList![0].id!);
                    // controllerText[index].text =
                    //     widget.model!.prVarientList![0].unit_set!;
                    if (tempId != null) {
                      controllerText[index].text = tempId.qty!;
                      print(
                          'Bulll${data.cartList[0].id} and ${tempId.qty} and ${tempId.id}');
                    } else {
                      controllerText[index].text =
                          widget.model!.prVarientList![0].unit_set!;
                      print('Nulll');
                            ...
                            Expanded(
                              child: NotificationListener<
                                ...
                                onNotification: (notif) {
                                  double requiredPixels =
                                      0.4 * notif.metrics.maxScrollExtent;
                                        ...
                                        [
                                          Column(
                                            ...
                                            children: <Widget>[
                                                  ...
                                                  Container(
                                                    ...
                                                    child: Column(
                                                      ...
                                                      children: [
                                                                    ...
                                                                    setState(
                                                                      ...
                                                                      var userId = context
                                                                          .read<
                                                                              UserProvider>()
                                                                          .userId;
                                                                      // Toggle the visibility of the QuantitySelector
                                                                      print(
                                                                          ' user id ${userId == null}');
                                                                      if (context
                                                                              .read<UserProvider>()
                                                                    ...
                                                                    children: <Widget>[
                                                                      InkWell(
                                                                        ...
                                                                        onTap:
                                                                            () {
                                                                          if (isProgress == false &&
                                                                              (int.parse(controllerText[index].text) > 0)) {
                                                                            removeFromCartDetail(widget.index);
                                                                          }
                                                                        },
                                                                      ),
                                                                      Container(
                                                                          ...
                                                                          children: [
                                                                            ...
                                                                            PopupMenuButton<String>(
                                                                              ...
                                                                              onSelected: (String value) {
                                                                                if (isProgress == false) {
                                                                                  addToCartDeatail(widget.index, value, 2);
                                                                                }
                                                                              },
                                                                              itemBuilder: (BuildContext context) {
                                                                                return widget.model!.itemsCounter!.map<PopupMenuItem<String>>(
                                                                                  (String value) {
                                                                      ...
                                                                      InkWell(
                                                                        ...
                                                                        onTap:
                                                                          ...
                                                                          if (isProgress ==
                                                                            ...
                                                                            addToCartDeatail(
                                                                                index,
                                                                                (int.parse(controllerText[index].text) + int.parse(widget.model!.prVarientList![0].unit_set.toString())).toString(),
                                                                                2);
                                                                          }
                                                                        },
                                                                      )
                                                            ...
                                                            children: List.generate(
                                                              ...
                                                              final parsedUnitSet =
                                                                  int.tryParse(
                                                                      unitSet ??
                                                                          '');
                                                              final parsedPrVarientUnitSet =
                                                                  int.tryParse(
                                                                      productSet
                                                                              .minimumQuantity ??
                                                                          '');
                                                              final minimumQuantity = productSet
                                                                          .minimumQuantity !=
                                                                      null
                                                                  ? int.parse(
                                                                      productSet
                                                              ...
                                                              bool isChecked =
                                                                  false; // Initialize isChecked here
                                                              if (indexOfProduct !=
                                                                  -1) {
                                                                // Product found in cartList
                                                                final qtyString =
                                                                    tempId?.qty ??
                                                                        unitSet;
                                                                if (qtyString !=
                                                                  ...
                                                                  if (qtyValue !=
                                                                    ...
                                                                    // Handle the case where qtyString is not a valid numeric value
                                                                  }
                                                                } else {
                                                                  // Handle the case where qtyString is null
                                                                }
                                                              }
                                                                    ...
                                                                    children: [
                                                                      ...
                                                                      Row(
                                                                        children: [
                                                                          ...
                                                                          const Text(
                                                                              'x'),
                                                                          Text(
                                                                            " ${productSet.sellingPriceSet ?? 'N/A'}=",
                                                                            style:
                                                                                const TextStyle(
                                                                              fontSize: 16,
                                                                              fontWeight: FontWeight.bold,
                                                                      ...
                                                                      Checkbox(
                                                                        ...
                                                                        onChanged:
                                                                          ...
                                                                          setState(
                                                                              () {
                                                                            print('check${controllerText[index].text}');
                                                                          });
                                                                        },
                                                                      ),
                                                                    ],
                                                        ...
                                                        ), // Replace the existing code with the QuantitySelector widget

                                                        // Display QuantitySelector if showQuantitySelector is true
                                                      ],
                                                    ),
                                                  ),
                                              ...
                                              //                   8.0,
                                              //               childAspectRatio: 3 /
                                              //                   4, // You can adjust the aspect ratio as needed
                                              //             ),
  ...
  Future<void> addToCartSelector(int index, String qty, int from) async {
    isNetworkAvail = await isNetworkAvailable();
    if (isNetworkAvail) {
      if (context.read<UserProvider>().userId != '') {
        if (mounted) {
  ...
  //                                       shrinkWrap: true,
  //                                       itemCount:
  //                                           widget.model!.productSets!.length,
  //                                       itemBuilder: (context, index) {
  //                                         bool isChecked =
  //                                             true; // Add your condition here to determine whether the checkbox should be checked or not.
  //                                         String? unitSet = widget.model!
  //                                             .prVarientList![0].unit_set;
  //                                         print(unitSet);
  //                                         /* assign your String value here */
  //                                         int? parsedUnitSet =
  //                                             int.tryParse(unitSet ?? '');
  //                                         int? parsedPrVarientUnitSet =
  //                                             int.tryParse(widget
  //                                                     .model!
  //                                                     .productSets![index]
  //                                                     .minimumQuantity ??
  //                                                 '');
  //                                         int minimumQuantity = widget
  //                                                     .model
  //                                                     ?.productSets![index]
  //                                                     .minimumQuantity !=
  //                                                 null
  ...
  //                                                 '');
  //                                         int first =
  //                                             parsedUnitSet! * minimumQuantity;
  //                                         num result = first * sellingPriceSet!;
  ...
  //                                                     const Text('x'),
  //                                                     Text(
  //                                                       " ${widget.model!.productSets != null && widget.model!.productSets!.isNotEmpty ? (widget.model!.productSets![index].sellingPriceSet!) : 'N/A'}=",
  //                                                       style: const TextStyle(
  //                                                         fontSize: 16,
  //                                                         fontWeight:
  //                                                             FontWeight.bold,
  ...
  //                                       // Update your logic for adding or removing items from the cart
  //                                       if (newQuantity > 0) {
  ...
  //               context,
  //               PageRouteBuilder(
  //                 pageBuilder: (_, __, ___) => ProductdetailsMore(
  //                   model: model,
  //                   secPos: 0,
  //                   index: index,
  //                   list: false,
    ...
    if (isNetworkAvail) {
      try {
        ...
        ApiBaseHelper().postAPICall(getProductApi, parameter).then(
          (getdata) {
            print(
                'API Response: of get $getdata'); // Add this line to print the response
  ...
  //     print('Error building short link: ${e.message}');
  //     // You can display an error message to the user
  //   } catch (e) {
  //     // Handle other exceptions
  //     print('Error building short link: $e');
  //     // You can display an error message to the user
  //   }
  // }

  Future<void> shareUrl(String url) async {
    // Use platform-specific share functionality
